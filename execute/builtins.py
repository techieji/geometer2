...
def _install_builtins():
    ...
    def add(args: list[EvalResult], env: ChainMap[Any, Any]) -> EvalResult:
        nums = [to_num(a) for a in args]
        result = sum(nums[1:], nums[0])
        return Token(TokenType.NUMBER, result)
    _bind_builtin('+', add)

    def sub(args: list[EvalResult], env: ChainMap[Any, Any]) -> EvalResult:
        nums = [to_num(a) for a in args]
        if len(nums) == 1:
    ...
    def mul(args: list[EvalResult], env: ChainMap[Any, Any]) -> EvalResult:
        nums = [to_num(a) for a in args]
        result = 1
        for n in nums:
            result *= n
    ...
    def div(args: list[EvalResult], env: ChainMap[Any, Any]) -> EvalResult:
        nums = [to_num(a) for a in args]
        if len(nums) == 1:
    ...
    def eq(args: list[EvalResult], env: ChainMap[Any, Any]) -> EvalResult:
        return Token(TokenType.BOOLEAN, args[0] == args[1])
    _bind_builtin('=', eq)

    def lt(args: list[EvalResult], env: ChainMap[Any, Any]) -> EvalResult:
        return Token(TokenType.BOOLEAN, to_num(args[0]) < to_num(args[1]))
    _bind_builtin('<', lt)

    def gt(args: list[EvalResult], env: ChainMap[Any, Any]) -> EvalResult:
        return Token(TokenType.BOOLEAN, to_num(args[0]) > to_num(args[1]))
    _bind_builtin('>', gt)

    def lte(args: list[EvalResult], env: ChainMap[Any, Any]) -> EvalResult:
        return Token(TokenType.BOOLEAN, to_num(args[0]) <= to_num(args[1]))
    _bind_builtin('<=', lte)

    def gte(args: list[EvalResult], env: ChainMap[Any, Any]) -> EvalResult:
        return Token(TokenType.BOOLEAN, to_num(args[0]) >= to_num(args[1]))
    _bind_builtin('>=', gte)

    def and_op(args: list[EvalResult], env: ChainMap[Any, Any]) -> EvalResult:
        for a in args:
            if not _is_truthy(a):
    ...
    def or_op(args: list[EvalResult], env: ChainMap[Any, Any]) -> EvalResult:
        for a in args:
            if _is_truthy(a):
                return a
        return Token(TokenType.BOOLEAN, False)
